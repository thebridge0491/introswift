...
import class Introswift_Intro.Intro
...
// - parameter progname: program name
 ...
 - returns: n/a */
func runIntro(_ progname:String, _ name:String) {
    do {
...
public func main() {
    ...
    if let jsonDict = try! Intro.readJSONObject(
			Intro.getTextFmFile("\(rsrcPath)/prac.json") ?? "{}") as? [String: Any] {
	...
	if let yamlDict = try? Yaml.load(
			Intro.getTextFmFile("\(rsrcPath)/prac.yaml") ?? "") {
	...
	if let yamlDict1 = try! Yams.load(yaml:
			Intro.getTextFmFile("\(rsrcPath)/prac.yaml") ?? "") as? [String: Any] {
